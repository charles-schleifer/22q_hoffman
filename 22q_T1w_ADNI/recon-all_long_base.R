...
sessions <- list.files(recondir, pattern = "Q_[0-9]")
...
subs_unique <- sort(unique(sesh_info$subject))

# for each subject, construct freesurfer longitudinal base command with all timepoints
fscall_base <- "recon-all -sd /u/project/cbearden/data/22q_T1w_all/sessions_recon-all/"
for(sub in subs_unique){
  fscall <- paste(fscall_base, "-base", sub)
  sub_all <- filter(sesh_info, subject==sub)
  sub_ordered <- sub_all[order(as.Date(sub_all$date)),]
  scans_ordered <- sub_ordered$MRI_S_ID
  for(scan in scans_ordered){
    fscall <- paste(fscall,"-tp",scan)
  }
  fscall <- paste(fscall,"-all")
  # create script to submit freesurfer command
  fname <- paste0(sub,"_recon-all_long_base.sh")
  ...
  qsub_command <- paste("qsub -cwd -V -o", file.path(logdir,"recon-all_log_base.$(date +%s).o"), "-e", file.path(logdir,"recon-all_log_base.$(date +%s).e"), "-l h_data=8G,h_rt=24:00:00,arch=intel*", fpath)
  system(qsub_command)
}
