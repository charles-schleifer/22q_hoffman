...
# Script to compute within network thalamic-cortical connectivity
...
main_compute_net_TC <- function(sesh, sessions_dir, bold_name_use, after_dir, file_end){
  ...
  net_tc <- lapply(c(1:12), function(v) roi_to_roi_fc(bold_mat1 = cort_mats[[v]], bold_mat2 = thal_mats[[v]])) %>% do.call(rbind,.) %>% as.data.frame
  colnames(net_tc) <- "TC_Fz"
  #colnames(net_rsfa) <- c("t_sd","t_mean")
  
  # merge with ji net key
  fc_result <- cbind(ji_net_keys,net_tc)
  
  # save result
  out_path <- file.path(dirname(rest_path),paste(bold_name_use,"_fc_network_Thal_Cortex",gsub(".dtseries.nii","",file_end),"_CABNP.csv",sep=""))
...
run_sesh_list <- function(sessions_dir,sesh_pattern,after_dir,bold_name_use,file_end,exclude=""){
  ...
  lapply(sesh_use, function(s) main_compute_net_TC(sesh=s, sessions_dir=sessions_dir, bold_name_use=bold_name_use, after_dir=after_dir, file_end=file_end))
}
