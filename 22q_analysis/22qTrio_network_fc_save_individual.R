# C. Schleifer 4/4/2022
# Script to compute functional connectivity between whole brain resting-state networks 
...
# function to compute Fz correlation between each unique pair of grayordinates in an ROI
within_roi_mean_fullmat <- function(bold_roi_mat){
  # crossing for each row of extracted BOLD timeseries matrix
  cross <- crossing_lower_tri_num(a=1:nrow(bold_roi_mat), b=1:nrow(bold_roi_mat))
  # for each row of cross df, correlate the timeseries in the bold_mask rows indicated by cross$a and cross$b
  fc <- mclapply(1:nrow(cross), function(r) FisherZ(as.numeric(cor(x=as.numeric(bold_roi_mat[cross[r,"a"],]), y=as.numeric(bold_roi_mat[cross[r,"b"],]), method="pearson", use="na.or.complete")))) %>% do.call(rbind,.) %>% as.numeric %>% as.data.frame
  colnames(fc) <- pearson_r_fz
  output <- cbind(cross,fc)
  #output <- as.data.frame(cbind(mean(fc), sd(fc), sd(fc)/sqrt(nrow(cross))))
  #colnames(output) <- c("fc_mean","fc_standard_dev","fc_standard_err")
  #print(output)
  print(".....ROI DONE")
  return(output)
}

#roi_to_brain_fc <- function

# function to compute FC checking for rows of all zero and skipping those cases (otherwise will get NA for those rows) 
fz_check_zeros <- function(x,y){
  if(!all(as.numeric(x == 0))){
    fc <- FisherZ(as.numeric(cor(x=x, y=y, method="pearson", use="na.or.complete")))
    return(fc)
  }else{
    print("WARNINIG: row with all zeros found. Skipping this vertex")
    # don't include this row in result
  }
}
...
within_roi_mean_gbc <- function(bold_roi_mat){
  ...
  # for each row of bold_roi_mean df, correlate the timeseries in the bold_mask rows indicated by cross$a and cross$b
  fc <- mclapply(1:nrow(bold_roi_mat), function(r) fz_check_zeros(x=as.numeric(bold_roi_mat[r,]),y=as.numeric(bold_roi_mean))) %>% do.call(rbind,.) %>% as.vector
  output <- as.data.frame(cbind(mean(fc), sd(fc), sd(fc)/sqrt(length(fc)), length(fc)))
  colnames(output) <- c("fc_mean","fc_standard_dev","fc_standard_err","n_grayordinates")
  print(output)
  return(output)
}

## wrapper for above functions to compute motion-scrubbed BETWEEN NETWORK fc for single subject
...
main_compute_bnet_fc <- function(sesh, sessions_dir, bold_name_use, after_dir, file_end){
  ...
  print("...extracting signal from CAB-NP networks")
  wb_ji_timeseries <- extract_rois_timeseries(bold_dtseries=bold_scrub, rois_dscalar=xii_Ji_network, rois_values=c(1:12), name_prefix="wb_")
  
  # compute FC
  print("...computing FC")
  fc_result <- roi_fc_matrix(as.data.frame(wb_ji_timeseries))
  print(paste(sesh,"DONE",paste=" "))
  
  # save result
  out_path <- file.path(dirname(rest_path),paste("resting_fc_matrix",gsub(".dtseries.nii","",file_end),"_whole_brain_CABNP_between_net.csv",sep=""))
...
## wrapper for above functions to compute motion-scrubbed WITHIN NETWORK gbc for single subject
main_compute_wnet_fc <- function(sesh, sessions_dir, bold_name_use, after_dir, file_end){
  print(paste("STARTING:", sesh, sep=" "))
  ...
  # extract from each CAB-NP network
  print("...calculating within-network FC for CAB-NP networks")
  net_mats <- lapply(c(1:12), function(v) extract_voxels_within_roi_timeseries(val=v,bold_dtseries=bold_scrub,rois_dscalar=xii_Ji_network))
  within_net_fc <- lapply(c(1:12), function(v) within_roi_mean_gbc(bold_roi_mat = net_mats[[v]])) %>% do.call(rbind,.) %>% as.data.frame

  # merge with ji net key
  fc_result <- cbind(ji_net_keys,within_net_fc)
  
  # save result
  out_path <- file.path(dirname(rest_path),paste("resting_fc_means",gsub(".dtseries.nii","",file_end),"_whole_brain_CABNP_within_net.csv",sep=""))
  print(paste("...saving result to:",out_path,sep=" "))
  write.table(fc_result, file=out_path, col.names=T, row.names=F, quote=F, na="NA", sep=",", eol = "\n")
  #return(fc_result)
}
...
#sesh="D17"
#sessions_dir=file.path(hoffman,"Enigma/Rome/qunex_studyfolder/sessions")
#bold_name_use="resting"
#after_dir="images/functional"
#file_end = "_Atlas_s_hpss_res-mVWMWB1d_lpss.dtseries.nii"

## wrapper for above functions to compute motion-scrubbed BETWEEN AND WITHIN NETWORK gbc for single subject
main_compute_bwnet_fc <- function(sesh, sessions_dir, bold_name_use, after_dir, file_end){
  print(paste("STARTING:", sesh, sep=" "))
  ...
  # compute FC
  print("...computing between-network FC")
  fc_resultb <- roi_fc_matrix(as.data.frame(wb_ji_timeseries))
  print(paste(sesh,"DONE",paste=" "))
  
  # save result
  out_pathb <- file.path(dirname(rest_path),paste("resting_fc_matrix",gsub(".dtseries.nii","",file_end),"_whole_brain_CABNP_between_net.csv",sep=""))
  print(paste("...saving result to:",out_pathb,sep=" "))
  write.table(fc_resultb, file=out_pathb, col.names=T, row.names=F, quote=F, na="NA", sep=",", eol = "\n")
  ...
  # merge with ji net key
  fc_resultw <- cbind(ji_net_keys,within_net_fc)
  
  # save result
  out_pathw <- file.path(dirname(rest_path),paste("resting_fc_means",gsub(".dtseries.nii","",file_end),"_whole_brain_CABNP_within_net.csv",sep=""))
  print(paste("...saving result to:",out_pathw,sep=" "))
  write.table(fc_resultw, file=out_pathw, col.names=T, row.names=F, quote=F, na="NA", sep=",", eol = "\n")
}




## wrapper for above functions to compute motion-scrubbed WITHIN NETWORK fc full matrix for single subject
#main_compute_wnet_fc_fullmat <- function(sesh, sessions_dir, bold_name_use, after_dir, file_end){
#  print(paste("STARTING:", sesh, sep=" "))
#  
#  # get %udvarsme from images/functional/movement/boldn.scrub
#  percent_udvarsme_all <- get_percent_udvarsme(sesh=sesh,sessions_dir=sessions_dir,bold_name_use=bold_name_use) %>% as.data.frame
...
#  print("...calculating within-network FC for CAB-NP networks")
#  net_mats <- lapply(c(1:12), function(v) extract_voxels_within_roi_timeseries(val=v,bold_dtseries=bold_scrub,rois_dscalar=xii_Ji_network))
#  within_net_fc <- lapply(c(1:12), function(v) within_roi_mean_fullmat(bold_roi_mat = net_mats[[v]])) %>% do.call(rbind,.) 
#  names(within_net_fc) <- ji_net_keys$NETWORK
#  
#  # merge with ji net key
#  #fc_result <- cbind(ji_net_keys,within_net_fc)
#  
#  # save result
#  out_path <- file.path(dirname(rest_path),paste("resting_fc_means",gsub(".dtseries.nii","",file_end),"_whole_brain_CABNP_within_net_all_row_pairs.RData",sep=""))
#  print(paste("...saving result to:",out_path,sep=" "))
#  #write.table(fc_result, file=out_path, col.names=T, row.names=F, quote=F, na="NA", sep=",", eol = "\n")
#  save(within_net_fc, file=out_path)
#}
#
...
run_sesh_list <- function(sessions_dir,sesh_pattern,after_dir,bold_name_use,file_end){
  ...
  #lapply(sesh_use, function(s) main_compute_bnet_fc(sesh=s, sessions_dir=sessions_dir, bold_name_use=bold_name_use, after_dir=after_dir, file_end=file_end))
  #lapply(sesh_use, function(s) main_compute_wnet_fc(sesh=s, sessions_dir=sessions_dir, bold_name_use=bold_name_use, after_dir=after_dir, file_end=file_end))
  lapply(sesh_use, function(s) main_compute_bwnet_fc(sesh=s, sessions_dir=sessions_dir, bold_name_use=bold_name_use, after_dir=after_dir, file_end=file_end))
}


# wrapper to only run fullmatrix fc (takes a lot of time so want to run it after the other results are done)
# currently skipping this function and just using gbc (each voxel to ROI average) for within-network
#run_sesh_list_fullmat <- function(sessions_dir,sesh_pattern,after_dir,bold_name_use,file_end){
#  print("Preparing to compute FC for all sessions in sessions_dir with existing input data")
#  print(paste("sessions_dir =",sessions_dir))
#  print(paste("sesh_pattern =",sesh_pattern))
#  print(paste("after_dir =",after_dir))
...
#  missing_files <- rest_path_all$fpath[which(exists_tf == F)]
#  
#  # calculate within network full fc matrix for all sessions with data (sesh_use)
#  lapply(sesh_use, function(s) main_compute_wnet_fc_fullmat(sesh=s, sessions_dir=sessions_dir, bold_name_use=bold_name_use, after_dir=after_dir, file_end=file_end))
#}
...
### within network full matrix
## GSR
# 22qTrio
#run_sesh_list_fullmat(sessions_dir = file.path(hoffman,"22q/qunex_studyfolder/sessions"),
#              sesh_pattern = "Q_[0-9]",
#              bold_name_use = "resting",
#              after_dir ="/images/functional/",
#              file_end = "_Atlas_s_hpss_res-mVWMWB1d_lpss.dtseries.nii")
#
## SUNY
#run_sesh_list_fullmat(sessions_dir = file.path(hoffman,"Enigma/SUNY/qunex_studyfolder/sessions"),
#              sesh_pattern = "X[0-9]",
#              bold_name_use = "resting",
#              after_dir ="/images/functional/",
#              file_end = "_Atlas_s_hpss_res-mVWMWB1d_lpss.dtseries.nii")
#
## IoP
#run_sesh_list_fullmat(sessions_dir = file.path(hoffman,"Enigma/IoP/qunex_studyfolder/sessions"),
#              sesh_pattern = "GQAIMS[0-9]",
#              bold_name_use = "resting",
#              after_dir ="/images/functional/",
#              file_end = "_Atlas_s_hpss_res-mVWMWB1d_lpss.dtseries.nii")
#
## Rome
#run_sesh_list_fullmat(sessions_dir = file.path(hoffman,"Enigma/Rome/qunex_studyfolder/sessions"),
#              sesh_pattern = "[0-9]",
#              bold_name_use = "resting",
#              after_dir ="/images/functional/",
#              file_end = "_Atlas_s_hpss_res-mVWMWB1d_lpss.dtseries.nii")
...
# qsub -cwd -V -o /u/project/cbearden/data/22q/qunex_studyfolder/processing/logs/manual/22qTrio_network_fc_save_individual.o -e /u/project/cbearden/data/22q/qunex_studyfolder/processing/logs/manual/22qTrio_network_fc_save_individual.e -l h_data=32G,h_rt=120:00:00,arch=intel*,highp /u/project/cbearden/data/22q/qunex_studyfolder/analysis/scripts/submit_within_network_GBC.sh 
