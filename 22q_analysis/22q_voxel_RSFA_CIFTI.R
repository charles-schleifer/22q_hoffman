...
# Script to compute voxelwise RSFA
# Inputs: preprocessed BOLDs, motion scrubbing file, paths and filenames
# Should be submitted as an array on hoffman using a wrapper script to submit one job per subject (submit_voxel_RSFA.sh -> run_voxel_RSFA.sh -> 22q_voxel_RSFA_CIFTI.R)
...
# functions to compute voxel-wise temporal SD and mean for RSFA and return CIFTI
cifti_voxel_sd <- function(xii){
  xii_out <- apply_xifti(xii, margin=1, sd)
  return(xii_out)
}
cifti_voxel_mean <- function(xii){
  xii_out <- apply_xifti(xii, margin=1, mean)
  return(xii_out)
}
...
# function to compute means from extracted parcels
roi_means <- function(bold_dscalar,rois_dscalar,val){
  if(ncol(bold_dscalar) != 1){
    stop("ERROR: bold input must have one column")
  }
  roi_vec <- extract_voxels_within_roi_timeseries(bold_dtseries=bold_dscalar,rois_dscalar=rois_dscalar,val=val) %>% as.vector
  roi_mean <- mean(roi_vec)
  return(roi_mean)
}


## wrapper for above functions to compute motion-scrubbed network RSFA for single subject
main_compute_voxel_rsfa <- function(sesh, sessions_dir, bold_name_use, after_dir, file_end){
  ...
  # calculate temporal standard deviaton at every vertex
  print("...calculating voxel-wise RSFA")
  bold_sd <- cifti_voxel_sd(bold_scrub)
  bold_mean <- cifti_voxel_mean(bold_scrub)
  
  # save result
  out_path_sd <- file.path(dirname(rest_path),paste0(bold_name_use,"_voxel_temporal_sd",file_end))
  out_path_m <- file.path(dirname(rest_path),paste0(bold_name_use,"_voxel_temporal_mean",file_end))
  print(paste("...saving results to:", out_path_sd))
  write_xifti(xifti=bold_sd, cifti_fname=out_path_sd)
  write_xifti(xifti=bold_mean, cifti_fname=out_path_m)
  
  # parcellate
  # extract from each CAB-NP parcel
  print("...extracting within-parcel RSFA for CAB-NP parcels")
  sd_parc_mean <- lapply(c(ji_key$INDEX), function(v) roi_means(val=v,bold_dscalar=bold_sd,rois_dscalar=xii_Ji_parcel)) %>% do.call(rbind,.) %>% as.data.frame
  colnames(sd_parc_mean) <- c("t_sd")
  m_parc_mean <- lapply(c(ji_key$INDEX), function(v) roi_means(val=v,bold_dscalar=bold_mean,rois_dscalar=xii_Ji_parcel)) %>% do.call(rbind,.) %>% as.data.frame
  colnames(m_parc_mean) <- c("t_mean")
  
  # merge with ji key
  fc_result <- cbind(ji_key[,c("INDEX","LABEL","HEMISPHERE","NETWORK","NETWORKKEY","GLASSERLABELNAME")],sd_parc_mean, m_parc_mean)
  
  # save result
  out_path <- file.path(dirname(rest_path),paste(bold_name_use,"_parcellated_voxel_RSFA",gsub(".dtseries.nii","",file_end),"_whole_brain_CABNP.csv",sep=""))
...
# do work
print("Preparing to compute voxel-wise RSFA...")
...
main_compute_voxel_rsfa(sesh=sesh, sessions_dir=sessions_dir, bold_name_use=bold_name_use, after_dir=after_dir, file_end=file_end)
