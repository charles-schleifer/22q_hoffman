...
# function to compute Fz correlation between roi and vector
roi_to_vec_fc <- function(bold_mat, bold_vec){
  # get the mean time series for each ROI
  bold_mean <- apply(bold_mat,2,mean)
  # correlate
  fc <- FisherZ(as.numeric(cor(x=bold_mean, y=bold_vec, method="pearson", use="na.or.complete")))
  return(fc)
}
...
main_compute_net_GBC <- function(sesh, sessions_dir, bold_name_use, after_dir, file_end){
  ...
  print("...calculating correlation between each CAB-NP network and nuisance WB timeseries")
  ...
  nuisance <-  read.csv(file.path(sessions_dir,sesh,after_dir,paste0(bold_name_use,"_movement_scrubbed_nuisance_regressors.csv")))
  wb_nuisance <- nuisance$WB %>% as.vector
  
  net_gbc <- lapply(c(1:12), function(v) roi_to_vec_fc(bold_mat = cort_mats[[v]], bold_vec = wb_nuisance)) %>% do.call(rbind,.) %>% as.data.frame
  ...
  # save result
  out_path <- file.path(dirname(rest_path),paste(bold_name_use,"_fc_network_Cortex_RawGBC",gsub(".dtseries.nii","",file_end),"_CABNP.csv",sep=""))
...
run_sesh_list <- function(sessions_dir,sesh_pattern,after_dir,bold_name_use,file_end,exclude=""){
  print("Preparing to compute network to WB connectivity for all sessions in sessions_dir with existing input data")
...
# qsub -cwd -V -o /u/project/cbearden/data/22q/qunex_studyfolder/processing/logs/manual/22q_RawGBC_save_individual.$(date +%s).o -e /u/project/cbearden/data/22q/qunex_studyfolder/processing/logs/manual/22q_RawGBC_save_individual.$(date +%s).e -l h_data=32G,h_rt=24:00:00,arch=intel* /u/project/cbearden/data/22q/qunex_studyfolder/analysis/scripts/submit_networkRawGBC.sh 
