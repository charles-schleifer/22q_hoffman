...
# Script to compute follow-up analyses after within-network thalamacortical connectivity
# for Frontoparietal network, compute whole thalamus connectivity and freesurfer thalamic atlas connectivity 
...
# TODO
# read example freesurfer thalamic parcellation from a subject (previously converted from mgz --> MNI152 nii --> dscalar) 
xii_thalparc_test <- read_cifti(file.path(hoffman,"22q/qunex_studyfolder/sessions/Q_0001_09242012/hcp/Q_0001_09242012/T1w/Q_0001_09242012/mri/ThalamicNuclei.dscalar.nii"), brainstructures = "all")
view_xifti_volume(xii_thalparc_test, slices=c(35,36,38,39,40,41,42,43,44), zlim=c(8100,8250), color_mode = "qualitative", colors=c("grey",rainbow(50)))
...
## wrapper for above functions to compute motion-scrubbed whole thalamus to cortical network FC for single subject
main_compute_wholethal_net_TC <- function(sesh, sessions_dir, bold_name_use, after_dir, file_end){
  ...
  print("...calculating whole-thalamus connectivity for CAB-NP networks")
  ...
  thal_mat <- extract_voxels_within_roi_timeseries(val=1,bold_dtseries=bold_scrub,rois_dscalar=thal_mask)
  
  net_tc <- lapply(c(1:12), function(v) roi_to_roi_fc(bold_mat1 = cort_mats[[v]], bold_mat2 = thal_mat)) %>% do.call(rbind,.) %>% as.data.frame
  colnames(net_tc) <- "whole_thal_TC_Fz"
  ...
  # save result
  out_path <- file.path(dirname(rest_path),paste(bold_name_use,"_fc_network_WholeThal_Cortex",gsub(".dtseries.nii","",file_end),"_CABNP.csv",sep=""))
...
run_sesh_list <- function(sessions_dir,sesh_pattern,after_dir,bold_name_use,file_end,exclude=""){
  ...
  lapply(sesh_use, function(s) main_compute_wholethal_net_TC(sesh=s, sessions_dir=sessions_dir, bold_name_use=bold_name_use, after_dir=after_dir, file_end=file_end))
}
...
# qsub -cwd -V -o /u/project/cbearden/data/22q/qunex_studyfolder/processing/logs/manual/22q_wholethal_TCC_save_individual.$(date +%s).o -e /u/project/cbearden/data/22q/qunex_studyfolder/processing/logs/manual/22q_wholethal_TCC_save_individual.$(date +%s).e -l h_data=32G,h_rt=24:00:00,arch=intel* /u/project/cbearden/data/22q/qunex_studyfolder/analysis/scripts/submit_networkTCwholethal.sh 
