...
packages <- c("conflicted", "magrittr", "dplyr","readxl", "data.table", "stringr", "tictoc","parallel")
...
qcfc_gsr_bl <- mclapply(roipairs, function(r)qcfc_roi(df=all_bparc_gsr_bl, roi=r, movement=movement),mc.cores=4) %>% do.call(rbind,.) %>% as.data.frame
...
qcfc_nogsr_bl <- mclapply(roipairs, function(r)qcfc_roi(df=all_bparc_nogsr_bl, roi=r, movement=movement),mc.cores=4) %>% do.call(rbind,.) %>% as.data.frame
...
#qsub -cwd -V -o /u/project/cbearden/data/22q/qunex_studyfolder/processing/logs/manual/qcfc.$(date +%s).o -e /u/project/cbearden/data/22q/qunex_studyfolder/processing/logs/manual/qcfc.$(date +%s).e  -pe shared 4 -l h_data=32G,h_rt=24:00:00 /u/project/cbearden/data/22q/qunex_studyfolder/analysis/scripts/submit_qcfc_mc.sh 
