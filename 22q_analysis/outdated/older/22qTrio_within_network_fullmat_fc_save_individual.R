...
within_roi_mean_gbc <- function(bold_roi_mat){
  ...
  output <- as.data.frame(cbind(mean(fc), sd(fc), sd(fc)/sqrt(nrow(bold_roi_mat))))
  colnames(output) <- c("fc_mean","fc_standard_dev","fc_standard_err")
  print(output)
  return(output)
}
...
#
## IoP
#run_sesh_list(sessions_dir = file.path(hoffman,"Enigma/IoP/qunex_studyfolder/sessions"),
...
#
## Rome
#run_sesh_list(sessions_dir = file.path(hoffman,"Enigma/Rome/qunex_studyfolder/sessions"),
#              sesh_pattern = "[0-9]",
#              bold_name_use = "resting",
#              after_dir ="/images/functional/",
#              file_end = "_Atlas_s_hpss_res-mVWMWB1d_lpss.dtseries.nii")
...
# qsub -cwd -V -o /u/project/cbearden/data/22q/qunex_studyfolder/processing/logs/manual/22qTrio_within_network_fullmat_fc_save_individual.o -e /u/project/cbearden/data/22q/qunex_studyfolder/processing/logs/manual/22qTrio_within_network_fullmat_fc_save_individual.e -l h_data=32G,h_rt=200:00:00,arch=intel*,highp /u/project/cbearden/data/22q/qunex_studyfolder/analysis/scripts/submit_within_network_fullmat.sh 
