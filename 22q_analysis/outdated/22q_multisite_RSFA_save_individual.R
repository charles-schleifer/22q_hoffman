...
# Script to compute parcellated RSFA
...
# function to compute average RSFA (resting state fluctuation amplitude) for an ROI
roi_rsfa <- function(bold_roi_mat){
  # mean for ROI
  ...
  if("matrix" %in% class(bold_roi_mat)){
    ...
    if(is.null(nrow(bold_roi_mat))){
      out <- cbind(NA,NA) %>% as.data.frame
      colnames(out) <- c("t_sd","t_mean")
    }else{
      bold_roi_mean <- apply(bold_roi_mat,2,mean)
      # for the mean ROI time series, return the temporal standard deviation and temporal mean in separate columns
      tsd <- sd(bold_roi_mean)
      tmean <- mean(bold_roi_mean)
      out <- cbind(tsd,tmean) %>% as.data.frame
      colnames(out) <- c("t_sd","t_mean")
    }
  # if ROI is single voxel, input will be numeric vector not matrix, and apply mean is not necessary
  }else if("numeric" %in% class(bold_roi_mat)){
    bold_roi_mean <- bold_roi_mat
    # for the mean ROI time series, return the temporal standard deviation and temporal mean in separate columns
    tsd <- sd(bold_roi_mean)
    tmean <- mean(bold_roi_mean)
    out <- cbind(tsd,tmean) %>% as.data.frame
...
## wrapper for above functions to compute motion-scrubbed network RSFA for single subject
main_compute_net_rsfa <- function(sesh, sessions_dir, bold_name_use, after_dir, file_end){
  ...
  print("...calculating within-network RSFA for CAB-NP networks")
  ...
  net_rsfa <- lapply(c(1:12), function(v) roi_rsfa(bold_roi_mat = net_mats[[v]])) %>% do.call(rbind,.) %>% as.data.frame
  #colnames(net_rsfa) <- "RSFA"
  colnames(net_rsfa) <- c("t_sd","t_mean")
  
  # merge with ji net key
  fc_result <- cbind(ji_net_keys,net_rsfa)
  
  # save result
  out_path <- file.path(dirname(rest_path),paste(bold_name_use,"_network_RSFA",gsub(".dtseries.nii","",file_end),"_whole_brain_CABNP.csv",sep=""))
...
main_compute_parc_rsfa <- function(sesh, sessions_dir, bold_name_use, after_dir, file_end){
  print(paste("STARTING:", sesh, sep=" "))
  ...
  print("...calculating within-parcel RSFA for CAB-NP parcels")
  ...
  parc_rsfa <- lapply(c(ji_key$INDEX), function(v) roi_rsfa(bold_roi_mat = parc_mats[[v]])) %>% do.call(rbind,.) %>% as.data.frame
  colnames(parc_rsfa) <- c("t_sd","t_mean")
  
  # merge with ji key
  fc_result <- cbind(ji_key[,c("INDEX","LABEL","HEMISPHERE","NETWORK","NETWORKKEY","GLASSERLABELNAME")],parc_rsfa)
  
  # save result
  out_path <- file.path(dirname(rest_path),paste(bold_name_use,"_parcel_RSFA",gsub(".dtseries.nii","",file_end),"_whole_brain_CABNP.csv",sep=""))
  print(paste("...saving result to:",out_path,sep=" "))
  write.table(fc_result, file=out_path, col.names=T, row.names=F, quote=F, na="NA", sep=",", eol = "\n")
  #return(fc_result)
}
...
# function to run RSFA for all sessions matching sesh_pattern in session_dir that have the file specified by after_dir, bold_name_use, and file_end
# sesh_pattern = "Q_[0-9]"
run_sesh_list <- function(sessions_dir,sesh_pattern,after_dir,bold_name_use,file_end,exclude=""){
  print("Preparing to compute RSFA for all sessions in sessions_dir with existing input data")
  ...
  print("computing parcel rsfa")
  lapply(sesh_use, function(s) main_compute_parc_rsfa(sesh=s, sessions_dir=sessions_dir, bold_name_use=bold_name_use, after_dir=after_dir, file_end=file_end))
  print("computing network rsfa")
  lapply(sesh_use, function(s) main_compute_net_rsfa(sesh=s, sessions_dir=sessions_dir, bold_name_use=bold_name_use, after_dir=after_dir, file_end=file_end))
}
...
# qsub -cwd -V -o /u/project/cbearden/data/22q/qunex_studyfolder/processing/logs/manual/22q_RSFA_save_individual.$(date +%s).o -e /u/project/cbearden/data/22q/qunex_studyfolder/processing/logs/manual/22q_RSFA_save_individual.$(date +%s).e -l h_data=32G,h_rt=48:00:00,highp,arch=intel* /u/project/cbearden/data/22q/qunex_studyfolder/analysis/scripts/submit_RSFA.sh 
