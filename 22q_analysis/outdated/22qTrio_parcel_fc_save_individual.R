...
within_roi_mean_gbc <- function(bold_roi_mat){
  ...
  if(!is.null(nrow(bold_roi_mat))){
    ...
    fc <- mclapply(1:nrow(bold_roi_mat), function(r) fz_check_zeros(x=bold_roi_mat[r,]), y=as.numeric(bold_roi_mean), method="pearson", use="na.or.complete") %>% do.call(rbind,.) %>% as.vector
...
## wrapper for above functions to compute motion-scrubbed BETWEEN AND WITHIN PARCEL gbc for single subject 
main_compute_bwparc_fc <- function(sesh, sessions_dir, bold_name_use, after_dir, file_end){
  print(paste("STARTING:", sesh, sep=" "))
  ...
  # compute FC
  print("...computing between-parcel FC")
  fc_resultb <- roi_fc_matrix(as.data.frame(wb_jiParc_timeseries))
  print(paste(sesh,"DONE",paste=" "))
  
  # save result
  out_pathb <- file.path(dirname(rest_path),paste("resting_fc_matrix",gsub(".dtseries.nii","",file_end),"_whole_brain_CABNP_between_parc.csv",sep=""))
  ...
  # merge with ji net key
  fc_resultw <- cbind(ji_key,within_parc_fc)
  
  # save result
  out_pathw <- file.path(dirname(rest_path),paste("resting_fc_means",gsub(".dtseries.nii","",file_end),"_whole_brain_CABNP_within_parc.csv",sep=""))
...
run_sesh_list <- function(sessions_dir,sesh_pattern,after_dir,bold_name_use,file_end){
  ...
  lapply(sesh_use, function(s) main_compute_bwparc_fc(sesh=s, sessions_dir=sessions_dir, bold_name_use=bold_name_use, after_dir=after_dir, file_end=file_end))
}
...
# qsub -cwd -V -o /u/project/cbearden/data/22q/qunex_studyfolder/processing/logs/manual/22qTrio_parcel_fc_save_individual.o -e /u/project/cbearden/data/22q/qunex_studyfolder/processing/logs/manual/22qTrio_parcel_fc_save_individual.e -l h_data=32G,h_rt=310:00:00,arch=intel*,highp /u/project/cbearden/data/22q/qunex_studyfolder/analysis/scripts/submit_parcel_fc.sh 

# qsub -cwd -V -o /u/project/cbearden/data/22q/qunex_studyfolder/processing/logs/manual/22qTrio_bparcel_fc_save_individual.o -e /u/project/cbearden/data/22q/qunex_studyfolder/processing/logs/manual/22qTrio_bparcel_fc_save_individual.e -l h_data=32G,h_rt=200:00:00,arch=intel*,highp /u/project/cbearden/data/22q/qunex_studyfolder/analysis/scripts/submit_bparcel_fc.sh 
# qsub -cwd -V -o /u/project/cbearden/data/22q/qunex_studyfolder/processing/logs/manual/22qTrio_wparcel_fc_save_individual.o -e /u/project/cbearden/data/22q/qunex_studyfolder/processing/logs/manual/22qTrio_wparcel_fc_save_individual.e -l h_data=32G,h_rt=200:00:00,arch=intel*,highp /u/project/cbearden/data/22q/qunex_studyfolder/analysis/scripts/submit_wparcel_fc.sh 